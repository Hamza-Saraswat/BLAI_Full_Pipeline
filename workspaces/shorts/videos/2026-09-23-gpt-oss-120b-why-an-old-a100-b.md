---
slug: 2026-09-23-gpt-oss-120b-why-an-old-a100-b
workspace: shorts
title: "gpt-oss-120b: why an old A100 beats an H100"
status: idea
pillar: explainer
structure: ""
format: classic
style_pack: ""
value_types: "TEACHES,REFRAMES"
created: 2026-09-23
updated: "2026-09-23T11:11:09Z"
publish_slot: ""
seo_score: 0
feedback: ""
blocked_reason: ""
build_host: ""
preview_url: ""
youtube_url: ""
blotato_post_id: ""
---
# gpt-oss-120b: why an old A100 beats an H100

## Artifacts
- Radar: [[stages/01-radar/output/2026-09-23-radar]]
- Ideas: [[stages/02-ideas/output/2026-09-23-ideas]]
- Research: (filled by stage 03)
- Script: (filled by stage 04)
- Package: (filled by stage 05)
- Voice: (filled by stage 06)
- Render: (filled by stage 07)
- Publish: (filled by stage 08)

## Decisions

- 2026-09-23 ideas stage: picked at opportunity 55.9; second slot over rank 3 (Qwen-Image-2.1, 74.3) because how-to repeats yesterday's lane and Qwen-Image-2.1 was yesterday's published product (30-day dedupe). Numbers must come from the cited Ornn paper only ($0.12/M output tokens, 5.1B active params, A100 cheaper than H100) -- no PROVES tag without our own measurement.
- Format classic: one fact and one consequence; the number does the work on its own.

## Build journal

