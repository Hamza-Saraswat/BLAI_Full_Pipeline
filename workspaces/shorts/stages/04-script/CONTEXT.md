...
3. Write **draft A and draft B in parallel**, one worker each, each in its own private scratch directory (`.local-builds/[slug]/draft-A/` and `draft-B/`; never a shared scratch path -- finding 11 lost work to a collision). A worker receives only: the brief, the voice-rule sections, signature-analogies.md, its own structure row, its assigned hook pattern, its band row and the Shorts platform row. Neither worker sees the other's draft. Unattended (Hermes): a worker is one `tools/llm_call.py --provider moonshot-k3 --model kimi-k3` call whose system file is the writer packet and whose user file is the inputs above; never a delegated subagent (the 2026-08-30 walk paid ~2.6M tokens for three of them).
...
6. Judge the survivors per judge-rubric.md (unattended: a third `llm_call.py` call on kimi-k3, the rubric as the system file, both storyboards as the user file). Apply any graft the rubric allows. Write `output/[slug]-drafts.md`.
