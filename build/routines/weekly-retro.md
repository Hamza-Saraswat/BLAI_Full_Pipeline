...
6. Commit the snapshot and the retro note to main from the repo root: `tools/git-sync.sh "analytics: retro WEEK" workspaces/shorts skills/render-shorts/styles/history.json`.
...
9. If a step fails (no `YT_API_KEY`, the API returns an error, fewer than two snapshots exist), do not stop silently: make sure `analytics/WEEK.md` exists and append a `## Blocked` section with the step and the exact error, then commit it with `tools/git-sync.sh "analytics: retro WEEK blocked" workspaces/shorts skills/render-shorts/styles/history.json`.
