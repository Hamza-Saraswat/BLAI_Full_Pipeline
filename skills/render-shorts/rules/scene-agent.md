...
- Safe area 900 x 1160 centered. Nothing that matters in the bottom 450 px (YouTube UI) or the right 120 px (like/comment rail). Each renderer has a debug helper (`showSafeZones` in Remotion, `safe_zone_debug()` in Manim, the `.safe` container in HyperFrames); use it when unsure.
...
8. Verify before handing back: `ffprobe` (1080x1920, 30 fps, duration), three stills (start, middle, end: text inside the safe area, at most 8 words, brand colors only, matches the brief), `python3 scripts/safe_zone_check.py <mp4> --scene`, `python3 scripts/lint_video.py <mp4>` (scene mode: 2 to 25 s, no audio needed).
9. Write only your own scene file and its render. Never touch the storyboard, the narration audio, captions, timing or other scenes.
