> BLAI note (v2 port): this folder is the scene-render template of `skills/render-shorts`. Read `../rules/scene-agent.md` and `../rules/hyperframes-1.md` first. The upstream skills named below live under `../vendor/` (only `hyperframes-core`, `hyperframes-cli` and `hyperframes-keyframes` are vendored; the video workflows are not installed). `npm install` once here; scenes are authored as `index.html` with the pack stylesheets in `packs/`.
