...
The dataset is the product. A Professional Voice Clone (PVC) trained on 30-60 minutes of clean, consistent narration sounds like the creator; one trained on noisy, mixed-style audio sounds like a worse stock voice. Plan: Creator plan (one PVC slot), trained on this dataset, used with Multilingual v2. Source: research section 3.5.
...
3. Read in the publishing register. The clone replicates the speaking style in the samples. Read real Shorts scripts (the `narration_full` of storyboards under `workspaces/shorts/stages/04-script/output/`) at the pace and energy the channel publishes with; the clone will not add energy later.
